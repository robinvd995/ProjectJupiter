...
namespace Jupiter::Xml {
	...
	float XmlNode::getValueAsFloat() const {
		return (float)atof(m_Node->value());
	}
	...
	float XmlNode::getAttributeAsFloat(const char* key) const {
		return (float)atof(m_Node->first_attribute(key)->value());
	}
	...
	const char* XmlNode::addStrSource(const std::string& value) {
		size_t curSize = m_Document->m_Xml->source_buffer.size();
		size_t newSize = curSize + value.size();
		if (newSize > JPT_XML_SOURCE_BUFFER_SIZE) { /*SLD_CORE_ERROR("Xml source buffer exceeded the predetermined max size!");*/ }
		m_Document->m_Xml->source_buffer.resize(newSize);
